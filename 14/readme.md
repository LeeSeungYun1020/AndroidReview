...
예를 들어 미국에 있더라도 온도를 화씨 단위가 아닌 섭씨로 확인하거나 일요일이 주의 시작이 아니라 월요일이 주의 시작이 되도록 설정하고 싶을 수도 있습니다.
...
Android 14 부터는 웹 컨텐츠 접근성 가이드라인(WCAG)을 준수하여 추가 접근성 기능을 포함하는 저시력자를 사용자를 위한 최대 200% 폰트 확대를 지원합니다.
...
앱의 폰트 사이즈가 알맞게 조절되고 큰 텍스트로 인해 사용성을 해치지 않는지 확인하기 위해 폰트 사이즈를 200%로 설정 후 UI 테스트를 진행하세요.

##### 텍스트 사이즈 지정 시 sp 단위 사용

텍스트 사이즈는 항상 sp 단위로 설정해야 합니다.
앱이 sp 단위를 사용하면, 안드로이드가 사용자가 선호하는 적합한 텍스트 크기와 비율을 적용할 수 있습니다.

padding이나 view의 높이를 지정할 때는 sp 단위를 사용하지 마세요.
폰트 확대는 비선형적으로 이루어질 수 있으므로 비율이 일치하지 않을 수 있습니다.
...
새로운 추가 PackageInstaller API 제공으로 앱 스토어가 사용자 경험을 개선할 수 있도록 하였습니다.
...
이 기능으로 업데이트 소유권을 적용하면 업데이트 권한을 가진 설치 프로그램만 자동 업데이트를 설치할 수 있습니다.
따라서 업데이트 소유권 적용을 통해 사용자가 원하는 앱 스토어에서만 업데이트를 받도록 보장할 수 있습니다.
...
앱 스토어는 이 기능을 이용하여 사용자가 앱을 사용 중인 경우에도 앱의 새로운 기능을 원활하게 설치할 수 있습니다.

#### 사용자가 스크린샷을 찍을 때 감지

스크린샷 감지에 대한 보다 일관적인 경험을 제공하기 위해 Android 14에서 개인 정보를 보호하는 스크린샷 감지 API를 도입하였습니다.
API로 앱은 액티비티 단위로 콜백을 등록할 수 있습니다.
액티비티가 보이는 상태에서 스크린샷을 찍으면 해당 콜백이 호출되며 사용자에게 알려집니다.
단 콜백에 촬영한 스크린샷 이미지가 제공되는 것은 아니며 사용자가 스크린샷을 찍을 때 화면에 나타날 내용을 앱이 결정할 수 있습니다.
...
머터리얼 컴포넌트 디펜던시(MDC-Android)를 1.10.0 버전 이상으로 적용하면
...
플랫폼 API의 OnBackAnimationCallback 같은 유사한 항목을 지원합니다.
